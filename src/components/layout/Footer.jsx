...
const Footer = () => {
  ...
  return (
    <footer className="footer-snap relative bg-[#FDFDFD] pt-8 pb-4 border-t-[3px] overflow-hidden" style={{ borderTopColor: COLOR_GOLD }}>
      ...
      {/* SILUET LOGO */}
      <div className="absolute right-[-2%] bottom-[-5%] opacity-[0.03] pointer-events-none select-none">
        <img src={logoSiluet} alt="" className="w-40 md:w-64 grayscale rotate-12" />
      </div>

      <div className="relative z-10 grid grid-cols-1 md:grid-cols-[1.2fr_0.8fr_0.8fr_1.2fr] gap-x-8 gap-y-6 w-full px-4 sm:px-6 md:px-8 lg:px-[5%] mb-6 items-start">
        
        {/* KOLOM LOGO */}
        <div className="space-y-3">
          <img src={logo} alt="AS PUTRA" className="h-8 w-auto object-contain" />
          <p className="text-slate-400 leading-relaxed text-[11px] max-w-[240px]">
        ...
        {/* KOLOM PERUSAHAAN */}
        <div>
          <h4 className="text-[9px] uppercase tracking-[0.2em] mb-3 font-black text-slate-800 flex items-center gap-2">
            <span className="w-3 h-[1.5px]" style={{ backgroundColor: COLOR_GOLD }}></span>
            Perusahaan
          </h4>
          <ul className="space-y-1 text-[12px] text-slate-500 font-medium">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-slate-500 text-[11px] leading-snug mb-3">
              Jl. Jend. Sudirman No.125, Winduhaji, <br />
          ...
          <div className="flex flex-col border-t border-slate-100 pt-3">
            <span className="text-[8px] uppercase tracking-widest text-slate-400 font-bold">Hotline</span>
            <a href="tel:+62232123456" className="text-[14px] font-black text-[#0F1A3E] hover:text-[#FFC619] transition-colors">
              (0232) 123456
            </a>
          </div>

          {/* SOSMED (IG, TIKTOK, WA, FB) */}
          <div className="flex gap-2">
            {[
              ...
              <a key={social.id} href="#" className="w-7 h-7 rounded bg-slate-100 flex items-center justify-center text-slate-500 hover:text-white hover:bg-[#FFC619] transition-all shadow-sm">
      ...
      {/* FOOTER BOTTOM */}
      <div className="relative z-10 px-4 sm:px-6 md:px-8 lg:px-[5%]">
        <div className="flex flex-col md:flex-row justify-between items-center border-t border-slate-100 pt-3 mt-2 gap-2 text-slate-400">
          <p className="text-[8.5px] tracking-widest font-bold uppercase">
            &copy; 2026 AS PUTRA Group. All rights reserved.
          </p>
          <div className="flex items-center gap-1 opacity-50 italic">
            <span className="text-[7.5px] uppercase tracking-widest font-bold text-slate-500">Powered by</span>
            <span className="text-[9px] font-black text-[#0F1A3E]">Solid Project</span>
          </div>
        </div>
      </div>
    </footer>
